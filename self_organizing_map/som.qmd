...
Self organizing maps are a parametric method of dimensionality or feature reduction
of continuous features.
They can be used to organize high dimensional data into just two dimensions. Well, really we *could* use them to map our data from any dimensionality into any other dimensionality, but they're generally most helpful for reducing many dimensions into just one, two, or three dimensions for visualizations.

## Data

Before doing anything, let's prepare some data.
We'll be using a small dataset about Iris flowers.
It contains 3 classes of Irises, and each datapoint contains the measurements of the length and width of a flower's sepal and petals.
...
Since we're going to be doing a lot of vector math later using these measurements,
let's convert them into vectors right away.
...
Now that we have the data prepared, we can start with some of the fun stuff.
...
You'll also notice, since we had 4 features we wanted to investigate,
we had to create two distinct plots to visualize all of our data.
This is something we can alleviate using Self Organizing Maps!
We'll be using them to organize our 4 dimensional data into just 2 dimensions.

## Building a Self Organizing Map Step by Step

Alright, we've covered some information about SOMs and the data is ready.
Now we'll start building and fitting the actual map.
