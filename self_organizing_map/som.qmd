...
fig, ax = plt.subplots(4, 2, gridspec_kw=grid_kws, figsize=(10.3, 9))

anim_map = map.copy()
anim_data = df['vector'].tolist()
anim_data = anim_data
anim_lr = 0.3
...
def animate(i):
    ...
    mean_delta = map_delta(anim_map, sample).mean(axis=2)
    scale = neighbor_scale(bmu_idx, (10, 10))
    mean_scaled_delta = fit_to_sample(anim_map, sample, anim_lr).mean(axis=2)
    before = anim_map.mean(axis=2)
...
```{python}
fitted = fit_epoch(map, df["vector"], 0.05)
